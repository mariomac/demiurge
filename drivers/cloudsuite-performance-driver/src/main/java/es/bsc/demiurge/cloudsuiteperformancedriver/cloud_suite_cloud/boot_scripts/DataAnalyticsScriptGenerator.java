...
public class DataAnalyticsScriptGenerator {
    ...
    private static String generateDefaultVmScript(VmSize vmSize) {
        return "#cloud-config" + END_OF_LINE
                ...
                + getModifyMapredXmlCommand(vmSize.getRamGb()) + END_OF_LINE
    ...
    private static String getModifyMapredXmlCommand(int ramGb) {
    ...
    private static int calculateMaxMappers(int ramGb) {
        return Math.max(1, (ramGb - 4)/4);
    }

    private static int calculateMaxReducers(int ramGb) {
        return Math.max(1, (ramGb - 4)/4);
    }

    private static String getModifyHadoopEnvCommand() {
    ...
    private static String getHadoopStopAllCommand() {
        return " - [ " + HADOOP_HOME + "/bin/stop-all.sh]";
    }

    private static String getHadoopStartAllCommand() {
        return " - [ " + HADOOP_HOME + "/bin/start-all.sh]";
    }

    private static String getRunMahoutModelValidationCommand() {
