 ...
 */
public class ScoreCalculatorEstimators implements SimpleScoreCalculator<ClusterState> {
    ...
    private int calculateSoftScore(ClusterState solution) {
        ...
        for (Host host: solution.getHosts()) {
            ...
            result -= VmPlacementConfig.estimationModeller.get().getEstimation(
					Policy.ESTIMATOR_BASED.getEstimatorLabel(),host, solution.getVmsDeployedInHost(host));
        }
        return (int) result;
    }
