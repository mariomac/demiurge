 ...
 */
public class ScoreCalculatorPerfAwarePowerAwareWithTimeShifting implements SimpleScoreCalculator<ClusterState> {
    private Logger logger = LogManager.getLogger(ScoreCalculatorPerfAwarePowerAwareWithTimeShifting.class);
    ...
    public ScoreCalculatorPerfAwarePowerAwareWithTimeShifting() {
        ...
        this.totalEnergy = energyManager.readTotalEnergyCsvFile();
        this.greenEnergy = energyManager.readGreenEnergyCsvFile();
        this.RES = energyManager.readRESCsvFile();
        ...
        for(Host h : solution.getHosts()) {
            ...
            for (Vm vm : vms_in_host){
                ...
                if (vm.getExtraParameters().getBenchmark() == null){
                    ...
                    if (shiftableBenchmarks.contains(vm.getExtraParameters().getBenchmarkStr())){
                        int duration = vm.getExtraParameters().getRunningTime();
                        double deadline = vm.getExtraParameters().getPerformance();

                        if (deadline - duration > 0){

                            long deploymentTime = getBestFutureAllocation(duration, deadline);

                            if (deploymentTime > 0){
                                vm.setTimeForDeploy(deploymentTime);
                            }
  ...
  private long getBestFutureAllocation(int duration, double deadline){

      double bestAllocationScore = Double.MAX_VALUE;
      int deploymentTime = 0;
/*
      // for each window get the predictions and calculate score
      for (int i = 0; i < deadline; i ++){
          int vmArrivals = getFutureVmArrivals(i, duration);
          double futureWorkload = getWindowWorkload(i, duration);
          double futureGreenEnergy = getBestFutureGreenEnergy(i, duration);

          double currentScore = 30*vmArrivals + futureWorkload + futureGreenEnergy;
