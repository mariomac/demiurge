...
public class HardwareInfo {
    ...
    public static Map<String, HardwareInfo> loadFromConfig(String filePath, String fileConfigPath) {
        ...
        try{
            ...
            String contents = (f.exists() && !f.isDirectory()) ? 
                FileSystem.readFile(filePath) : FileSystem.readConfigurationFile(fileConfigPath);
            
            JsonNode jsonHwInfo = new ObjectMapper().readTree(contents);

            for (JsonNode node : jsonHwInfo) {
                hwinfo.put(node.get("hostname").asText(), new HardwareInfo(
                    node.get("hostname").asText(),
                    node.get("cpu").get("vendor").asText(),
                    node.get("cpu").get("arch").asText(),
                    node.get("disk").get("type").asText()
                ));
            }
        }
        catch(Exception e){
