...
public class DeploymentPlan {
     ...
     */
    public List<VirtualMachine> getVmsAssignedToHost(String hostname) {
    ...
    private boolean allHostsHaveEnoughResourcesForTheirAssignations() {
        for (Map.Entry<PhysicalHost, List<VirtualMachine>> entry: getMapOfHostsAndTheirVmsAssignations().entrySet()) {
     ...
     * @return the map
     */
    private Map<PhysicalHost, List<VirtualMachine>> getMapOfHostsAndTheirVmsAssignations() {
        Map<PhysicalHost, List<VirtualMachine> > result = new HashMap<>();
        for (VmAssignmentToHost vmAssignmentToHost: vmsAssignationsToHosts) {
            if (!result.containsKey(vmAssignmentToHost.getHost())) {
                result.put(vmAssignmentToHost.getHost(), new ArrayList<VirtualMachine>());
            }
