...
public class Scheduler {
     ...
     */
    public Scheduler(SchedAlgorithmNameEnum schedAlg, List<VmDeployed> vmsDeployed, List<Host> hosts) {
     ...
     */
    public DeploymentPlan chooseBestDeploymentPlan(List<Vm> vms) {
        String deploymentId = getDeploymentIdForLogMessages();
		Logger log = LogManager.getLogger(Scheduler.class);

		log.info("Start deployment " + deploymentId + " plan evaluation: " + schedAlgorithm.getNameEnum().getName());
        ...
        if (bestDeploymentPlan != null) {
			log.info("Chosen deployment " + deploymentId + " plan: " + bestDeploymentPlan.toString());
        }
        else { // No plans could be chosen, so apply overbooking
			log.info("Overbooking needed for deployment " + deploymentId);
        }
